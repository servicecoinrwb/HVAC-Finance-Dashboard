...
const App = () => {
    ...
    useEffect(() => {
        const seedData = async () => {
            const collections = ['hvac_customers', 'hvac_work_orders', 'hvac_technicians', 'hvac_invoices', 'hvac_quotes'];
            ...
            if (shouldSeed) {
                ...
                initialCustomers.forEach(c => batch.set(doc(db, 'artifacts', appId, 'public', 'data', 'hvac_customers', c.id), c));
                initialSampleData.forEach(wo => batch.set(doc(db, 'artifacts', appId, 'public', 'data', 'hvac_work_orders', wo.id), wo));
                initialTechnicians.forEach(t => batch.set(doc(db, 'artifacts', appId, 'public', 'data', 'hvac_technicians', t.id), t));
                initialInvoices.forEach(i => batch.set(doc(db, 'artifacts', appId, 'public', 'data', 'hvac_invoices', i.id), i));
                initialQuotes.forEach(q => batch.set(doc(db, 'artifacts', appId, 'public', 'data', 'hvac_quotes', q.id), q));
                await batch.commit();
            }
        };
        ...
        const run = async () => {
            ...
            onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'hvac_work_orders'), (snapshot) => setWorkOrders(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))));
            onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'hvac_customers'), (snapshot) => setCustomers(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))));
            onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'hvac_technicians'), (snapshot) => setTechnicians(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))));
            onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'hvac_invoices'), (snapshot) => setInvoices(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))));
            onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'hvac_quotes'), (snapshot) => setQuotes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))));
        };
        run();
    }, []);
    ...
    const handleUpdateOrder = async (orderId, payload) => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'hvac_work_orders', orderId), payload); setSelectedOrder(p => ({ ...p, ...payload })); };
    const handleAddNote = async (orderId, noteText, callback) => { if (!noteText.trim()) return; const newNote = { text: noteText.trim(), timestamp: new Date().toISOString() }; const order = workOrders.find(o => o.id === orderId); await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'hvac_work_orders', orderId), { notes: [...(order.notes || []), newNote] }); callback(); };
