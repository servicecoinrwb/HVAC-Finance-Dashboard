...
function formReducer(state, action) {
    switch (action.type) {
        ...
        case 'RESET_FORM':
            return action.payload; // payload will be the new initial state
        default:
            return state;
    }
}
...
const WorkOrderDetailModal = () => {
    ...
    const { selectedOrder, setSelectedOrder, technicians, handlers } = useWorkOrderContext();
    
    // Rename 'selectedOrder' to 'order' for convenience within this component
    const order = selectedOrder;

    const [formData, dispatch] = useReducer(formReducer, {});
    const [newNote, setNewNote] = useState('');

    // 4. Sync the form state when the selected order changes
    useEffect(() => {
        if (order) {
            dispatch({
                type: 'RESET_FORM',
    ...
    // Don't render the modal if no order is selected
    ...
    const handleAddNote = () => {
        if (newNote.trim()) {
            handlers.addNote(order.id, newNote, () => setNewNote(''));
        }
    };
                    ...
                    <section className="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex justify-end mt-2">
                                <button onClick={handleAddNote} disabled={!newNote.trim()} className="bg-green-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-green-700 disabled:bg-gray-400">Add Note</button>
                            </div>
                        </div>
                    </section>
                </main>
